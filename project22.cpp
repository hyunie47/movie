...
int main(int argc, char *argv[]) {
	...
	fgets(name, sizeof(name), fp); //1. reading the movie.dat-----------------------------
	FILE *fp = fopen("movie.dat", "name"); //1.1 FILE open
	...
	while(exit_flag == 0) 
	{
		...
		switch(option)
		{
			case 1: //print all the movies
				...
				ndPtr = list;
				while (for i=0; i<end; i++)
				{
					printf("%s, %s", movie.c, linkedList.c); //2.2 print a movie data : use functions of movie.c and linkedList.c
			...
			case 2: //print movies of specific country
				repfunc = mv_printCountry; //2.3.1 get country name to search for
